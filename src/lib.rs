...
/// tuples.
///
/// For example, a triply-nested keypath would be represented as an `Aggregator<(T1, (T2, T3))>`,
/// where `T3` is evaluated on the top-most item, then `T2`, then `T1.
...
/// The trait that powers indexing via key-path.
///
/// There are not separate traits for move-by-index, mut-ref-by-index, and ref-by-index.
/// Instead, `KeyPathIndexable` is implemented for `T`, `&T`, and `&mut T` separately.
...
pub trait MapKeyPath: Iterator {
	...
	where
		...
		Self: Sized
	{
		self.map(<Self::Item as KeyPathIndexable<KP>>::idx)
	}
}

impl<T> MapKeyPath for T where T: Iterator {}

/// A utility trait to facilitate some type fuckery used in the [`Keyable`] proc-macro
///
